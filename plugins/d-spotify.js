...
if (!text) return conn.sendMessage(m.chat, { text: `Ingrese el comando mas el nombre de la cancion o un enlace de una cancion de Spotify para descargarlo.\n\n• *Por ejemplo:*\n${usedPrefix + command} Nessa Barret` }, { quoted: m });
...
📍  Se esta descargando el audio, espere un momento...`;
...
await conn.sendMessage(m.chat, { audio: { url: info.url }, mimetype: "audio/mpeg", ptt: false, contextInfo: { externalAdReply: { title: track.name, body: `Artista : ${track.artists.map((a) => a.name).join(", ")}`, thumbnailUrl: track.album.images[0]?.url, mediaType: 1, mediaUrl: track.external_urls.spotify, sourceUrl: track.external_urls.spotify }}}, { quoted: m });
await conn.sendMessage(m.chat, { react: { text: "✅", key: m.key } });
} catch (err) {
console.error(err);
