...
if (!text) return conn.sendMessage(m.chat, { text: `Ingrese el comando y escriba lo que quiera buscar en pinterest o un enlace para descargarlo.\n\n• Por ejemplo:\n*#${command}* https://pin.it/5kpxKqFS7` }, { quoted: m })
try {
await m.react('⏳')
if (text.includes("https://")) {
let i = await dl(args[0])
let cPin = await pins(text)
let isVideo = i.download.includes(".mp4")
await conn.sendMessage(m.chat, { [isVideo ? "video" : "image"]: { url: i.download || cPin }, caption: "✓ Contenido descargado." }, { quoted: fkontak })
} else {
const results = await pins(text)
if (!results.length) {
return conn.sendMessage(m.chat, { text: `📍  No se ha encontrado resultados para la busqueda *[ ${text} ]*...` }, { quoted: m })
}
const medias = results.slice(0, 10).map(img => ({ type: 'image', data: { url: img.image_large_url } }))
conn.sendMessage(m.chat, { text: `✅  Imagenes encontradas en *Pinterest*, enviando...` }, { quoted: m })
await conn.sendSylphy(m.chat, medias, { caption: null, quoted: m })
}} catch (e) {
